...
class GMW_PT_Search_Query extends GMW {
    ...
    public function results() {
        ...
        if ($this->form['advanced_query']) {
            ...
            $intensiteit_args = array();
            $intensiteitLgz_args = array();

            $intensiteitMin = isset($_GET['intensiteitMin']) && $_GET['intensiteitMin'] != '' ? $_GET['intensiteitMin'] : '';
            $intensiteitMax = isset($_GET['intensiteitMax']) && $_GET['intensiteitMax'] != '' ? $_GET['intensiteitMax'] : '';

            $intensiteitLgzMin = isset($_GET['intensiteitLgzMin']) && $_GET['intensiteitLgzMin'] != '' ? $_GET['intensiteitLgzMin'] : '';
            $intensiteitLgzMax = isset($_GET['intensiteitLgzMax']) && $_GET['intensiteitLgzMax'] != '' ? $_GET['intensiteitLgzMax'] : '';

            $intensiteitSnelheidMin = isset($_GET['intensiteitSnelheidMin']) && $_GET['intensiteitSnelheidMin'] != '' ? $_GET['intensiteitSnelheidMin'] : '';
            $intensiteitSnelheidMax = isset($_GET['intensiteitSnelheidMax']) && $_GET['intensiteitSnelheidMax'] != '' ? $_GET['intensiteitSnelheidMax'] : '';

            $minJaar = intval($_GET['bokwold_metas']['jaar'] != '' ? $_GET['bokwold_metas']['jaar'] : intval($_GET['bokwold_metas']['maxjaar'] != '' ? 2000 : ''));
            $maxJaar = intval($_GET['bokwold_metas']['maxjaar'] != '' ? $_GET['bokwold_metas']['maxjaar'] : intval($_GET['bokwold_metas']['jaar'] != '' ? date("Y") : ''));

            $metas = isset($_GET['bokwold_metas']) ? $_GET['bokwold_metas'] : '';
            
            if($minJaar || $maxJaar) {
                $years = range($minJaar, $maxJaar);
                ...
                foreach ($years as $jaar) {
                    if ($intensiteitMin != '' || $intensiteitMax != '' || $intensiteitLgzMin != '' || $intensiteitLgzMax != '' || $intensiteitSnelheidMin != '' || $intensiteitSnelheidMax != '') {
                        // Normaal verkeer
                        if ($intensiteitMin != '' && $intensiteitMax != '') {
                            $intensiteit_args['relation'] = 'OR';
                            $intensiteit_args[] = array(
                                'key' => $jaar,
                                'value' => array((float) $intensiteitMin, (float) $intensiteitMax),
                                'compare' => 'BETWEEN',
                                'type' => 'DECIMAL',
                            );
                        } else if ($intensiteitMin != '') {
                            $intensiteit_args['relation'] = 'OR';
                            $intensiteit_args[] = array(
                                'key' => $jaar,
                                'value' => (float) $intensiteitMin,
                                'compare' => '>=',
                                'type' => 'DECIMAL',
                            );
                        } else if ($intensiteitMax != '') {
                            $intensiteit_args['relation'] = 'OR';
                            $intensiteit_args[] = array(
                                'key' => $jaar,
                                'value' => (float) $intensiteitMax,
                                'compare' => '<=',
                                'type' => 'DECIMAL',
                            );
                        }
                        ...
                        // Langzaam verkeer
                        if ($intensiteitLgzMin != '' && $intensiteitLgzMax != '') {
                            $intensiteitLgz_args['relation'] = 'OR';
                            $intensiteitLgz_args[] = array(
                                'key' => $jaar . '_lgz',
                                'value' => array((float) $intensiteitLgzMin, (float) $intensiteitLgzMax),
                                'compare' => 'BETWEEN',
                                'type' => 'DECIMAL',
                            );
                        } else if ($intensiteitLgzMin != '') {
                            $intensiteitLgz_args['relation'] = 'OR';
                            $intensiteitLgz_args[] = array(
                                'key' => $jaar . '_lgz',
                                'value' => (float) $intensiteitLgzMin,
                                'compare' => '>=',
                                'type' => 'DECIMAL',
                            );
                        } else if ($intensiteitLgzMax != '') {
                            $intensiteitLgz_args['relation'] = 'OR';
                            $intensiteitLgz_args[] = array(
                                'key' => $jaar . '_lgz',
                                'value' => (float) $intensiteitLgzMax,
                                'compare' => '<=',
                                'type' => 'DECIMAL',
                            );
                        }
                        ...
                        // Snelheid verkeer
                        if ($intensiteitSnelheidMin != '' && $intensiteitSnelheidMax != '') {
                            $intensiteitSnelheid_args['relation'] = 'OR';
                            $intensiteitSnelheid_args[] = array(
                                'key' => $jaar . '_snelheid',
                                'value' => array((float) $intensiteitSnelheidMin, (float) $intensiteitSnelheidMax),
                                'compare' => 'BETWEEN',
                                'type' => 'DECIMAL',
                            );
                        } else if ($intensiteitSnelheidMin != '') {
                            $intensiteitSnelheid_args['relation'] = 'OR';
                            $intensiteitSnelheid_args[] = array(
                                'key' => $jaar . '_snelheid',
                                'value' => (float) $intensiteitSnelheidMin,
                                'compare' => '>=',
                                'type' => 'DECIMAL',
                            );
                        } else if ($intensiteitSnelheidMax != '') {
                            $intensiteitSnelheid_args['relation'] = 'OR';
                            $intensiteitSnelheid_args[] = array(
                                'key' => $jaar . '_snelheid',
                                'value' => (float) $intensiteitSnelheidMax,
                                'compare' => '<=',
                                'type' => 'DECIMAL',
                            );
                        }
            ...
            if(count($intensiteit_args) > 1) {
                $meta_query_args[] = $intensiteit_args;
            }
            
            if(count($intensiteitLgz_args) > 1) {
                $meta_query_args[] = $intensiteitLgz_args;
            }
            
            if(count($intensiteitSnelheid_args) > 1) {
                $meta_query_args[] = $intensiteitSnelheid_args;
            }
            if ($metas) {
                foreach ($metas as $key => $value) {
                    if ($value == '' || $key == 'jaar' || $key == 'maxjaar' ) {
                        continue;
                    }
            ...
            //echo '<pre>';print_r($meta_query_args);
            //query args
            $this->form['query_args'] = array(
                ...
                'meta_query' => apply_filters('gmw_pt_meta_query', $meta_query_args, $this->form),
                'order' => 'ASC',
                'orderby' => 'title',
